...
public class Task2 {
    public static void main(String[] args) {
        ...
        if (rank == 0) {
            ...
            System.out.println("Неблокирующее получение к " + rank);
        }
        rq.Wait();
        ...
        if (rank == 0) {
            ...
            System.out.println("Одновременная отправка к " + nextRank + " и получение от " + prevRank);
            System.out.println("Финальная сумма: " + rankBuffer1[0]);
        } else {
            MPI.COMM_WORLD.Recv(rankBuffer, 0, 1, MPI.INT, prevRank, 0);
            System.out.println("Получение к " + rank);
            rankBuffer[0] += rank;
            MPI.COMM_WORLD.Send(rankBuffer, 0, 1, MPI.INT, nextRank, 0);
            System.out.println("Отправка от " + rank);
        }
